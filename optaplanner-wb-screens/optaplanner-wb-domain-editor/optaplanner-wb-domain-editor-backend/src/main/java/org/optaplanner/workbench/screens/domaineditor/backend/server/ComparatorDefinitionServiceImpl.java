...
public class ComparatorDefinitionServiceImpl implements ComparatorDefinitionService {
    ...
    public JavaClass createComparatorObject( DataObject dataObject ) {
        ...
        generatedAnnotation.setValue( "value", Arrays.asList( ComparatorDefinitionService.class.getName() ) );
        ...
        Method compareMethod = updateCompareMethod( new MethodImpl(), comparatorObject.getAnnotation( ComparatorDefinition.class.getName() ), dataObject.getClassName() );
    ...
    @Override
    public JavaClass updateComparatorObject( DataObject dataObject, JavaClass comparatorObject ) {
        ...
        comparatorObject.addMethod( updateCompareMethod( compareMethod.get(), comparatorObject.getAnnotation( ComparatorDefinition.class.getName() ), dataObject.getClassName() ) );

        return comparatorObject;
    }

    private Method updateCompareMethod( Method compareMethod, Annotation objectPropertyPaths, String type) {

        Parameter parameter1 = new ParameterImpl( new TypeImpl( type ), "o1" );
        Parameter parameter2 = new ParameterImpl( new TypeImpl( type ), "o2" );

        Type returnType = new TypeImpl( "int" );

        compareMethod.setName( "compare" );
        compareMethod.setParameters( Arrays.asList( parameter1, parameter2 ) );
        compareMethod.setBody( generateCompareBody( objectPropertyPaths, type ) );
    ...
    private String generateCompareBody( Annotation comparatorDefinition, String type ) {
        ...
        sb.append( getComparingRow( firstObjectPropertyPath, true, type ) );
        for ( int i = 1; i < objectPropertyPaths.size(); i++ ) {
            sb.append( getComparingRow( objectPropertyPaths.get( i ), false, type ) );
        }
    ...
    private String getComparingRow( Annotation objectPropertyPath, boolean firstElement, String type ) {
        StringBuilder sb = new StringBuilder();
        ...
        if ( fieldPaths != null ) {
            for ( Annotation pathElement : fieldPaths ) {
                ...
                if ( typeString.endsWith( ".class" ) ) {
                    typeString = typeString.substring( 0, typeString.indexOf( ".class" ) );
                }
                sb.append( "tempResult = tempResult == null ? null : " )
                        .append( "(( " )
                        .append( previousObjectPropertyClassName )
                        .append( ") tempResult )." )
                        .append( generationTools.toJavaGetter( (String) pathElement.getValue( "name" ), typeString ) )
                        .append( "();" );
